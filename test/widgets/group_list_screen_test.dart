...
Group _group({
  ...
  g.groupMembers =
      members ??
      [_member(_myEmail, isFavorite: favorite), _member('sam@test.com')];
    ...
    ProviderScope(
      overrides: [
        groupListProvider.overrideWith(
          () => _FakeGroupListNotifier(groups, toggled ?? []),
        ),
...
class _FakeUserNotifier extends UserDetailNotifier {
  ...
  Future<SupaUser> build() async =>
      const SupaUser(email: _myEmail, displayName: 'Alex');
}
...
void main() {
  ...
  testWidgets('renders greeting, group name and a card', (tester) async {
    await _pumpScreen(
      tester,
      groups: [_group(id: 'a', name: 'Trip to Rome', totalShareAmount: 25)],
    );

    expect(
      find.text('Alex'),
      findsOneWidget,
    ); // name line of the greeting header
  ...
  testWidgets(
    'greeting header renders a muted greeting line above the name line',
    (tester) async {
      await _pumpScreen(
        tester,
        groups: [_group(id: 'a', name: 'A', totalShareAmount: 10)],
      );
  ...
  testWidgets('hero owe/owed labels map to the sign of the net balance', (
    tester,
  ) async {
    final l10n = await AppLocalizations.delegate.load(const Locale('en'));
  ...
  testWidgets('hero shows the settled lead when the net balance is zero', (
    tester,
  ) async {
    final l10n = await AppLocalizations.delegate.load(const Locale('en'));
  ...
  for (final brightness in Brightness.values) {
    testWidgets(
      'overall-balance hero amount uses the big displayMedium tier ($brightness)',
      (tester) async {
        ...
        // group cards).
        final heroMoney = tester.widget<MoneyText>(
          find.byType(MoneyText).first,
        );
        ...
        expect(
          heroMoney.style?.fontSize,
          greaterThan(Theme.of(ctx).textTheme.displaySmall!.fontSize!),
        );
      },
    );
  }

  testWidgets('tapping the star toggles favorite and does not navigate', (
    tester,
  ) async {
  ...
  testWidgets('cards appear in favorite-sort order', (tester) async {
    ...
    final items = tester
        .widgetList<GroupListItem>(find.byType(GroupListItem))
        .toList();
    expect(items.map((i) => i.group.id).toList(), ['fav', 'active', 'settled']);
  });

  testWidgets('settled group card shows the gray label and no amount', (
    tester,
  ) async {
    await _pumpScreen(
      tester,
      groups: [_group(id: 's', name: 'Settled Group', totalShareAmount: 0)],
    );
  ...
  testWidgets('unsettled group card still shows the balance amount', (
    tester,
  ) async {
    await _pumpScreen(
      tester,
      groups: [_group(id: 'a', name: 'Active Group', totalShareAmount: 25)],
    );
  ...
  for (final brightness in Brightness.values) {
    testWidgets(
      'group cards show distinct per-group tints matching the palette ($brightness)',
      (tester) async {
        tester.view.physicalSize = const Size(1080, 2400);
        tester.view.devicePixelRatio = 3.0;
        addTearDown(tester.view.resetPhysicalSize);
  ...
  TextStyle leadLabelStyle(WidgetTester tester, String label) => tester
      .widget<Text>(
        find.descendant(
  ...
  for (final brightness in Brightness.values) {
    testWidgets(
      'owed card footer: muted caption lead label + green card-title amount ($brightness)',
      (tester) async {
        await _pumpScreen(
          tester,
          brightness: brightness,
  ...
  testWidgets('owe card footer: amount uses the danger (red) semantic token', (
    tester,
  ) async {
    await _pumpScreen(
      tester,
    ...
    expect(
      find.descendant(
        of: find.byType(GroupListItem),
        matching: find.text(l10n.balanceOwe),
      ),
      findsOneWidget,
    );
  ...
  testWidgets(
    'settled card footer: muted gray caption label, no colored amount',
    (tester) async {
      await _pumpScreen(
        tester,
  ...
  testWidgets(
    'many-member group + long German label keeps the amount inside the card (F152)',
    (tester) async {
      ...
      expect(
        ...
        lessThan(120),
        reason:
            '4 overlapped tokens must be far narrower than the old +16px-gap Row',
      );
  ...
  testWidgets(
    'group cards are fully visible after pumpAndSettle (stagger completes)',
    (tester) async {
      await _pumpScreen(
        tester,
        groups: [
  ...
  testWidgets(
    'group cards are visible immediately when disableAnimations is true (reduced motion)',
    (tester) async {
      final groups = [
        _group(id: 'a', name: 'Alpha Group', totalShareAmount: 10),
      ];
  ...
  testWidgets(
    'favorite toggle does not throw and keeps cards visible (no-replay guard)',
    (tester) async {
      final toggled = <String>[];
      final groups = [
        _group(id: 'a', name: 'Alpha Group', totalShareAmount: 10),
  ...
  testWidgets('no standalone New Group floating action button remains', (
    tester,
  ) async {
    await _pumpScreen(
      tester,
  ...
  testWidgets('the "+ New" section-header action creates a group', (
    tester,
  ) async {
  ...
  testWidgets('empty state offers a create-first-group CTA that navigates', (
    tester,
  ) async {
  ...
  // -------------------------------------------------------------------------
  // multi-currency-group: the overall-balance hero shows a per-currency
  // breakdown instead of converting into a home currency.
  // -------------------------------------------------------------------------

  testWidgets('an all-EUR home shows no expand affordance on the hero', (
    tester,
  ) async {
    await _pumpScreen(
      tester,
    ...
    final l10n = await AppLocalizations.delegate.load(const Locale('en'));
    // Not mounted at all — the disclosure AND the 8px spacer that precedes it
    // sit inside one guard, so an all-EUR hero keeps the exact height and line
    // count it has today rather than growing 8px.
    expect(find.byType(CurrencyBreakdownDisclosure), findsNothing);
    expect(find.byIcon(Icons.expand_more), findsNothing);

    // The hero column ends on the owed/owe stat row: no trailing spacer.
  ...
  testWidgets('no displayed hero figure is the sum of two currencies', (
    tester,
  ) async {
    await _pumpScreen(
      tester,
    ...
    // 3025.50 / 3025 would be the merged figure. Neither is anywhere on screen.
    expect(find.textContaining('3,025'), findsNothing);
    expect(find.textContaining('3025'), findsNothing);
  });

  testWidgets('a EUR group and a JPY group each render in their own currency '
      'with the right decimal digits', (tester) async {
    await _pumpScreen(
      tester,
      groups: [
        _group(id: 'eur', name: 'Flat', totalShareAmount: 25.50),
