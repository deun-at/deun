                  ...
                  Expanded(
                    child: _FriendAddResults(
                      ...
                      onRequestContactPermission: () {
                        ref
                            .read(friendAddProvider.notifier)
                            .retryContactPermission();
                      },
                    ),
                  ),
                ],
    ...
    return CustomScrollView(
      slivers: [
          ...
          child: Consumer(
            builder: (context, ref, _) {
              final searchResults = ref.watch(
                friendAddProvider.select((s) => s.searchResults),
              );
              final searchText = ref.watch(
                friendAddProvider.select((s) => s.searchText),
              );
              final isLoading = ref.watch(
                friendAddProvider.select((s) => s.isLoading),
              );
              final isAmbiguous = ref.watch(
                friendAddProvider.select((s) => s.isAmbiguousUsername),
              );
        ...
        SliverToBoxAdapter(
          child: Consumer(
            builder: (context, ref, _) {
              final contactSuggestions = ref.watch(
                friendAddProvider.select((s) => s.contactSuggestions),
              );
              final isLoading = ref.watch(
                friendAddProvider.select((s) => s.isLoading),
              );
              final permissionDenied = ref.watch(
                friendAddProvider.select((s) => s.contactPermissionDenied),
              );
