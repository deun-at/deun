...
✅ **done · fc0da38** — restyled `update_password.dart` (Screen 2): replaced opaque `SupaResetPassword` with a custom
form making the EXACT same call (`updateUser(UserAttributes(password:…))` → `go('/group')`), accent tile + Bricolage
title + soft inset password field w/ visibility toggle + 52px primary CTA. Restyled `onboarding_screen.dart` (Screen 3):
accent `alternate_email` tile, `SectionLabel` sections, `@username#code` inset row + live `@user#code` handle preview,
display-name field, bottom-pinned CTA; existing `saveUsername`/`onComplete`/`initialDisplayName` contract untouched.
Pure `onboarding_username.dart` (`sanitizeUsername`/`isValidUsername`/`previewHandle`, TDD). 15 tests; 4 en+de keys;
analyze clean, 526 green. *v0:* `#code` discriminator is server-generated at save, so live preview uses a muted `#0000`
placeholder (username portion updates live, lowercased to match what's stored). *QA:* Screen 2 tile/title/field/spinner;
Screen 3 accent tile, live handle preview color (`primary` bold) legibility, keyboard vs pinned CTA, long-username
overflow — both light+dark.
