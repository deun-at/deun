...
- [ ] F33 · Groups home · Google Mobile Ads SDK debug error banner ("This ad may have not been loaded or has been disposed. Ad with the following id could not be found: 0") paints as a red full-width block below the group list when the NativeAd fails to load; intermittently reappears on list rebuild even though NativeAdBlock returns SizedBox.shrink on failure 🔥 — lib/widgets/native_ad_block.dart:39 (mounted at lib/pages/groups/presentation/group_list.dart:184) — target: no red ad/debug banner ever visible on home; suppress/teardown the SDK debug platform-view on load-failure (deepens previously-closed F02) — ev: compare/compare_groups.png
